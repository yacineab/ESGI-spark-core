...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'rankLangs' should work for RDD with two elements") {
    ...
    val rdd = spark.sparkContext.parallelize(List(WikipediaArticle("1", "Scala is great"), WikipediaArticle("2", "Java is OK, but Scala is cooler")))
  ...
  test("'makeIndex' creates a simple index with two entries") {
    ...
    val rdd = spark.sparkContext.parallelize(articles)
